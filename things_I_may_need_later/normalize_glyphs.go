...
// toRelative converts baked absolute commands to relative form and
// appends a relative move so the current point ends at (8, 0) relative
// to the glyph origin.
func toRelative(cmds []Command) []Command {
	...
	// Append relative move so we end at (8, 0) relative to glyph origin
	dx := 8 - curX
	dy := 0 - curY
...
// ProcessGlyphPath does the full treatment: bake transforms + convert to relative.
...
// ProcessGlyphPathAbsolute bakes the transforms but keeps absolute commands.
func ProcessGlyphPathAbsolute(originalD string, tx, ty float64) string {
	cmds := ParsePath(originalD)
	baked := bake(cmds, tx, ty)
	return SerializePath(baked)
}

func main() {
	if len(os.Args) < 3 {
		fmt.Println("Usage:")
		fmt.Println("  go run normalize_glyphs.go input.html output.html [--absolute]")
		fmt.Println("")
		fmt.Println("  --absolute   Bake transforms only (keep absolute path commands)")
		fmt.Println("               Default: bake + convert to relative + final positioning")
		return
	}
	...
	for i := 2; i < len(os.Args); i++ {
		if os.Args[i] == "--absolute" {
			absoluteMode = true
		} else if os.Args[i] == "--go-map" && i+1 < len(os.Args) {
	...
	newContent := re.ReplaceAllStringFunc(content, func(match string) string {
		...
		newInner := pathRe.ReplaceAllStringFunc(inner, func(pmatch string) string {
			...
			var newD string
			if absoluteMode {
				newD = ProcessGlyphPathAbsolute(oldD, tx, ty)
			} else {
	...
	if svgOut != "" {
		...
		fmt.Printf("Wrote SVG: %s (absoluteMode=%v)\n", svgOut, absoluteMode)
	}
	...
	if goMapOut != "" {
		...
		if absoluteMode {
			b.WriteString("// glyphPaths: transforms baked in, absolute path commands preserved.\n")
		} else {
			b.WriteString("// glyphPaths: baked + relative paths with final positioning move.\n")
		}
